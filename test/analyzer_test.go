...
// TestAnalyzeWithLoops runs the analyzer with loop analysis enabled.
func TestAnalyzeWithLoops(t *testing.T) {
	results := runAnalyzer(t, parsercommands.AnalyzeOptions{AnalyzeLoops: true})
	checkGoldenCSV(t, results, "analyzer-report-with-loops.csv")

	// More detailed JSON file assertions
	checkGoldenJSON(t, results, []testCaseAssertion{
		{
...
// checkGoldenCSV sanitizes a copy of the analyzer's CSV results, then checks it against the golden file with the provided name (without the `.golden` suffix).
...
func checkGoldenCSV(t *testing.T, results *analyzerResults, goldenName string) {
	t.Helper()
...
// checkGoldenJSON checks the analyzer's JSON output for the specified test names against the corresponding golden files.
// This involves clearing the test's `project`, `filePath`, and `importPath` fields since they naturally vary between test runs.
// Note that the `goldie` library internally handles updating the generated golden files using the `-update` flag.
func checkGoldenJSON(t *testing.T, results *analyzerResults, cases []testCaseAssertion) {
	t.Helper()
	g := goldie.New(t, goldie.WithFixtureDir(goldenDir))
	...
	for _, tc := range cases {
		t.Run(tc.testName, func(t *testing.T) {
			...
			// TODO cleanup maybe unmarshal the JSON files directly into an AnalysisResult struct in `jsonFile()` to avoid the weird mapping,
