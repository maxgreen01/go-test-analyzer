...
These black-box tests compare the analysis engine's output against golden files (with suffix `.golden`) in [`test/testdata/`](./test/testdata/) to verify behavior. If the tool's internal logic changes, these files may become outdated and lead to test failures. In such cases, the golden files must be updated to reflect the new expected output. Golden files should only be modified with great care, as updating the golden files using faulty results could mask potential regressions!
